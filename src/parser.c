...
u8 ParseData(u8 copy_u8DataBuffer[]){
	...
	//check for ':'
	if(AsciToHex(copy_u8DataBuffer[parse_counter]) == ':'){
		parse_counter++;
		//continue parsing
		//get the CC
		DigitalCharHigh = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		DigitalCharLow  = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		CC = (DigitalCharHigh<<4) | DigitalCharLow;
		//get the address
		DigitalAddress3 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		DigitalAddress2 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		DigitalAddress1 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		DigitalAddress0 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		...
		if(RT == 1){	//if RT is end of file return the state that ends the bootloader program
			...
			for(u16 i=0; i<CC/2; i++){	// CC/2 because we get two bytes at once
				DigitalData3 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
				DigitalData2 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
				DigitalData1 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
				DigitalData0 = AsciToHex(copy_u8DataBuffer[parse_counter++]);
		...
		state = 1;
		return state;	//return the continue state (there're more data to send)
		
	}
	else
	{
		state = -1;
		return state; //return error
	}
}
